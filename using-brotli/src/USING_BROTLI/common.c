...
void printError(const CommonError* result) {
  ...
  switch (result->code) {
    ...
    case ErrOriginalSizeUnknown:
      ...
      break;
    case ErrNotBrotliFile:
      fprintf(stderr, "The file cannot be read because it wasn't compressed by Brotli.");
      break;
  }
...
void printHumanReadableSizeTo(FILE* outFile, const size_t fileSize) {
  ...
  fprintf(outFile, "%.1f%s", fileSizeDecimal, sizeNames[i]);
}

void printHumanReadableSize(const size_t fileSize) {
  printHumanReadableSizeTo(stdout, fileSize);
}
