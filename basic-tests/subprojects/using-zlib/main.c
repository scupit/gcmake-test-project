// This is a slightly modified version of the zpipe.c file
// presented as the zlib usage example at
  ...
  switch (ret) {
    case Z_ERRNO:
      if (ferror(stdin))
        ...
        fputs("Error with some buffer", stderr);
      break;
    ...
    default: printf("Unhandled error code %d\n", ret); break;
  }
}
...
int main(int argc, char** argv)
{
  ...
  if (argc == 2) {
    FILE* myFile = fopen(argv[1], "wb");
    returnCode   = def(stdin, myFile, Z_DEFAULT_COMPRESSION);
    fclose(myFile);
    ...
    fputs("zpipe usage: zpipe [-r] <fileName>\n", stderr);
    return 1;
  }
}
